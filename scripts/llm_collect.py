"""
LLM Daily Collector – běží v repozitáři sndbx
----------------------------------------------
Spouštění: python llm_collect.py
Výstup:    data/llm_data.jsonl  (append, 10 řádků/den)
"""
...
OUTPUT = Path(__file__).parent / "sndbx" / "llm_data.jsonl"
...
def group_share(records: list[dict]) -> dict[str, float | None]:
    vals  = {r["hf_id"]: (r["_hf"].get("downloads_alltime") or 0) for r in records}
...
def main():
    ...
    print(f"  {'#':<3} {'Model':<28} {'Param B':>8}  {'Podíl %':>8}  Architektura")
    print("  " + "─" * 65)
    for i, r in enumerate(
        sorted(records, key=lambda x: x["parameters_b"], reverse=True), 1
    ):
